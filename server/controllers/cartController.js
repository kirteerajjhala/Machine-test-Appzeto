...
const quantity = (value) => {
  if (!Number.isInteger(value) || value < 1 || value > 999)
    throw new ApiError(400, "Quantity must be between 1 and 999");
};
...
export const getCart = async (req, res) => {
  const cart = await load(req.user._id);
  res.json({
    success: true,
    data: { cart: render(cart || new Cart({ user: req.user._id, items: [] })) },
  });
};
export const addCartItem = async (req, res) => {
  ...
  if (item) {
    ...
    if (item.quantity + count > product.stock)
      throw new ApiError(400, "Requested quantity exceeds available stock");
    item.quantity += count;
  } else cart.items.push({ product: product._id, quantity: count });
  cart.lastActivityAt = new Date();
  await cart.save();
  res
    .status(201)
    .json({ success: true, data: { cart: render(await load(req.user._id)) } });
};
export const updateCartItem = async (req, res) => {
  ...
  cart.lastActivityAt = new Date();
  await cart.save();
  res.json({ success: true, data: { cart: render(await load(req.user._id)) } });
};
